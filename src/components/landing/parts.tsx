 ...
 * while it auto-advances, so visitors see that the scene is moving on its own
 * and can take over with a click at any point.
 */
export function SceneSteps({
  ...
  return (
    <div role="tablist" aria-label="Scene steps" className={cn('flex flex-wrap gap-2', className)}>
      {steps.map((s, i) => {
        const on = i === index
        return (
          <button
          ...
          >
            ...
            {on && playing && (
              <motion.span
                key={`bar-${index}`}
                className={cn('absolute bottom-0 left-0 h-[2px]', tone === 'lime' ? 'bg-lime' : 'bg-electric')}
                initial={{ width: '0%' }}
                animate={{ width: '100%' }}
