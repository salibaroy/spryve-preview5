...
import { ArrowRight, Bell, BookOpen, MessageCircle } from 'lucide-react'
import { DashboardMock, type MockModule } from './DashboardMock'
import { useAutoCycle } from '@/lib/hooks'
import { cn } from '@/lib/utils'

const SIGNALS: { id: string; text: string; module: MockModule; icon: typeof Bell; tone: 'lime' | 'blue'; pos: string }[] = [
  { id: 'deadline', text: 'Launchway Health Accelerator closes in 11 days', module: 'opps', icon: Bell, tone: 'lime', pos: 'lg:-left-10 lg:top-[46%]' },
  { id: 'message', text: 'Maya Sleiman sent you a message', module: 'messages', icon: MessageCircle, tone: 'blue', pos: 'lg:-right-6 lg:top-[70%]' },
  { id: 'lesson', text: 'Pricing fundamentals: lesson 3 is next', module: 'learning', icon: BookOpen, tone: 'blue', pos: 'lg:right-[18%] lg:-bottom-8' },
]

export function Hero() {
  const scene = useAutoCycle(SIGNALS.length, 2800, { loop: true })
  const active = SIGNALS[scene.index]
  ...
  return (
    <section className="relative overflow-hidden pt-[68px]">
      ...
      <div
        ...
        style={{ background: 'radial-gradient(closest-side, color-mix(in oklab, var(--brand-tertiary) 55%, transparent), transparent)' }}
        aria-hidden
      />
      ...
      <div className="container-site relative grid items-center gap-12 pt-12 pb-20 md:pt-16 lg:min-h-[calc(92svh-68px)] lg:grid-cols-[0.92fr_1.08fr] lg:gap-10 lg:pb-24">
        <div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.08 }} className="t-hero max-w-[14ch]">
          ...
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.2 }} className="lede mt-6">
            A dashboard that shows what matters next. A Founders Hub to meet the people building beside you. And Spryve Build,
            optional hands-on help when you want to design or develop something.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1], delay: 0.32 }} className="mt-8 flex flex-wrap gap-3">
        ...
        <motion.div ref={scene.ref} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: 0.25 }} className="relative">
          <div className="lg:rotate-[-1.2deg]">
            <DashboardMock focus={[active.module]} />
          </div>
